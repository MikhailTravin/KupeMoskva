...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
	echo json_encode(['message' => 'Только POST запросы']);
	exit;
}
...
foreach ($required_files as $file) {
	if (!file_exists($file)) {
		echo json_encode(['message' => 'Не найден файл: ' . basename($file)]);
		exit;
	}
}
...
try {
	...
	$mail->isHTML(false);
	$mail->CharSet = 'UTF-8';
	$mail->Subject = 'Новая заявка с квиза - ' . date('d.m.Y H:i');
	$mail->Body = $body;
	...
	if (strpos($error, 'authenticate') !== false || strpos($error, 'password') !== false) {
		try {
			...
			$mail2->isHTML(false);
			$mail2->CharSet = 'UTF-8';
			$mail2->Subject = '📋 Новая заявка с квиза - ' . date('d.m.Y H:i');
			$mail2->Body = $body;
